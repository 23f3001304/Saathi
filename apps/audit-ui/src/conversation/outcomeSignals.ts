...
export function outcomeSignals(
  ...
  if (beat.state === "answered") return ends();
  ...
  if (known === undefined)
    return [...ends(), ...settled, pill(`outcome-${beat.state}`, beat.state)];
  ...
  // A failure's detail is the provider's own error string, a stack trace
  // wearing a sentence; the harness's line stands alone.
  const machine =
    beat.state === "failed" ||
    beat.detail.match(/^(txn_|order_|rzp_)|no link issued/) !== null;
  const spoken = machine ? lead : `${lead} ${beat.detail}`.trim();
  return [...ends(), ...settled, { kind: "say", text: spoken, system: true }];
}

/** Every outcome closes the work block and idles the run. */
function ends(): AssistantSignal[] {
