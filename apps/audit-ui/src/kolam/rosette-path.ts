// §5.6 — the covenant seal: one continuous kolam line around a 3×3 pulli
// grid, generalising thread.ts's vertical-only arc formula to travel in any
// of the four grid directions so the boustrophedon order closes on itself.
type Pt = { x: number; y: number };
type Arc = { entry: Pt; exit: Pt; dir: Pt; sweep: 0 | 1 };

export const ROSETTE_ORDER: ReadonlyArray<readonly [number, number]> = [
  [0, 0],
  [1, 0],
  [2, 0],
  [2, 1],
...
/** One path, one `strokeDashoffset` animation, no lifting of the hand (§5.6). */
