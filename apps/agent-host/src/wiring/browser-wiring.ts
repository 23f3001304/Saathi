...
import {
  ...
  capFrom,
  queueLimitFor,
} from "../browser/session-capacity.js";
 ...
 */
/**
 * How many windows this host promises, said out loud at boot.
 *
 * Warm containers come out of the budget rather than sitting on top of it: one
 * that is waiting still holds its full memory entitlement, so a cap that
 * counted only claimed windows would promise the queue more than this host can
 * open. Never below one - a machine too small to keep anything warm must still
 * be able to open a window.
 */
function capacityOf(obs: ObsParts): number {
  const reserved = warmReserved(warmSizesFrom(process.env));
  const derived = capFor({
    dockerMemMb: Number(process.env["COVENANT_DOCKER_MEM_MB"] ?? 8192),
    cpus: cpus().length,
  });
  const room = capFrom(process.env, derived);
